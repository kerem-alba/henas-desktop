...
def get_detailed_seniority():
    ...
    cur.execute(
        """
        SELECT s.id, s.seniority_name, s.max_shifts_per_month, s.shift_duration,
        ...
        GROUP BY s.id, s.seniority_name, s.max_shifts_per_month, s.shift_duration
        ORDER BY s.id
        """
    )
    result = cur.fetchall()
        ...
        seniority_list.append(
            {
                ...
                "seniority_name": row[1],
                "max_shifts_per_month": row[2],
                "shift_duration": row[3],
                "shift_area_names": row[4] if row[4] is not None else [],
                "shift_area_ids": row[5] if row[5] is not None else []
            }
        )
